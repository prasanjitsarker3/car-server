...
async function run() {
    try {
        ...
        app.get('/services/:id', async (req, res) => {
            ...
            const result = await collectionService.findOne(query, options);
            res.send(result);
        })
        ...
        app.delete('/bookings/:id', async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) }
            const result = await collectionBooking.deleteOne(query);
            res.send(result)
        })
